...
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ url('/admin/activities') }}">{{ __('global.activity.news') }}</a></li>
    <li class="breadcrumb-item">@yield('title')</li>
@endsection

@section('content')
<form action="{{ url('/admin/activities/'.$row->id) }}" method="post">@csrf {{ method_field('PUT') }}
    <div class="row">

        <div class="col-md-4 mb-3">
            <label>{{ __('global.activity.categor_name') }} <strong class="text-danger">*</strong></label>
            <select name="category_id" class="form-control">
                @foreach ($cats as $cat)
                    <option @if($row->category_id==$cat->id) selected @endif value="{{$cat->id}}">{{ $cat->translation->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="col-md-4 mb-3">
            ...
            <div class="input-group mb-3" dir="ltr">
                <div class="input-group-prepend">
                    <span class="input-group-text" id="basic-addon1">{{url('activity')}}/</span>
                </div>
        ...
        <div class="col-md-4 mb-3">
            <label>{{ __('global.activity.url') }}</label>
            <div class="input-group mb-3" dir="ltr">
                <input type="url" class="form-control" value="{{old('url', $row->url)}}" name="url" />
            </div>
        </div>

        <div class="col-md-5 mb-3">
            <label>{{ __('global.activity.thum_img') }} <strong class="text-danger">*</strong></label>
        ...
        <div class="col-md-5 mb-3">
            <label>{{ __('global.activity.news_img') }} <strong class="text-danger">*</strong></label>
            <div class="input-group">
                <span class="input-group-btn">
        ...
        <div class="col-md-4 mb-3">
            <label>{{ __('global.activity.created_at') }} <strong class="text-danger">*</strong></label>
                            ...
                            <div class="tab-pane {{ ($val->locale==appLangKey())? 'active' : '' }}" id="tab-{{$val->locale}}">
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <label>{{ __('global.activity.news_name') }} ({{ LangNative($val->locale) }}) <strong class="text-danger">*</strong></label>
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <label>{{ __('global.activity.news_tags') }} ({{ LangNative($val->locale) }})</label>
                                        <br />
                                        <input name="tags[{{$val->locale}}]" value="{{ old("tags")[$val->locale] ?? $val->tags }}" class="tag_{{$val->locale}}" placeholder="{{ __('global.activity.news_tags') }}" />
                                    </div>

                                    <div class="col-md-6 mb-3">
                                        <label>{{ __('global.activity.news_description') }} ({{ LangNative($val->locale) }}) <strong class="text-danger">*</strong></label>
                                    ...
                                    <div class="col-md-12 mb-3">
                                        <label>{{ __('global.activity.news_content') }} ({{ LangNative($val->locale) }}) <strong class="text-danger">*</strong></label>
