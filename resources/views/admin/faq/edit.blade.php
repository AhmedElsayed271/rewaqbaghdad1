...
@section('title', __('global.faq_edit'))

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ url('/admin/faq') }}">{{ __('global.faq_title') }}</a></li>
    <li class="breadcrumb-item">@yield('title')</li>
@endsection

@section('content')
<form action="{{ url('/admin/faq/'.$row->id) }}" method="post">@csrf {{ method_field('PUT') }}
    <div class="row">
                            ...
                            <div class="tab-pane {{ ($val->locale==appLangKey())? 'active' : '' }}" id="tab-{{$val->locale}}">
                                <div class="row">
                                    ...
                                    <div class="col-md-12 mb-3">
                                        <label>{{ __('global.faq_question') }} ({{ LangNative($val->locale) }}) <strong class="text-danger">*</strong></label>
                                        <textarea dir="{{langDir($val->locale)}}" name="question[{{$val->locale}}]" class="form-control my-editor" rows="5">{!! old('content')[$val->locale] ?? $val->answer !!}</textarea>
                                    </div>

                                    <div class="col-md-12 mb-3">
                                        <label>{{ __('global.faq_answer') }} ({{ LangNative($val->locale) }}) <strong class="text-danger">*</strong></label>
                                        <textarea dir="{{langDir($val->locale)}}" name="answer[{{$val->locale}}]" class="form-control my-editor" rows="5">{!! old('content')[$val->locale] ?? $val->answer !!}</textarea>
                                    </div>
