...
@section('title', __('global.rewaq.rewaq_edit'))
...
<form action="{{ url()->current() }}" method="post">@csrf
    <div class="row">
        ...
        <div class="col-md-3 mb-3">
            <label>{{ __('global.rewaq.team.jobs_titles')[appLangKey()]['pm'] }} <strong class="text-danger">*</strong></label>
            <select required name="pm_id" class="form-control">
                @foreach ($team_pm as $pm)
                    <option @if($row->pm_id==$pm->id) selected @endif value="{{$pm->id}}">{{$pm->translation->name}}</option>
                @endforeach
            </select>
        </div>
        
        <div class="col-md-3 mb-3">
            <label>{{ __('global.rewaq.team.jobs_titles')[appLangKey()]['am'] }} <strong class="text-danger">*</strong></label>
            <select required name="am_id" class="form-control">
                @foreach ($team_am as $am)
                    <option @if($row->am_id==$am->id) selected @endif value="{{$am->id}}">{{$am->translation->name}}</option>
                @endforeach
            </select>
        </div>
        
        <div class="col-md-3 mb-3">
            <label>{{ __('global.rewaq.team.jobs_titles')[appLangKey()]['ps'] }} <strong class="text-danger">*</strong></label>
            <select required name="ps_id" class="form-control">
                @foreach ($team_ps as $ps)
                    <option @if($row->ps_id==$ps->id) selected @endif value="{{$ps->id}}">{{$ps->translation->name}}</option>
                @endforeach
            </select>
        </div>
