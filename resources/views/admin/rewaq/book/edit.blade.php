...
@section('content')
<form action="{{ url('/admin/rewaq-books/'.$row->id) }}" method="post">@csrf {{ method_field('PUT') }}
    <div class="row">
        ...
        {{-- <div class="col-md-5 mb-3">
            ...
            <input required dir="ltr" type="url" class="form-control" name="index_url" required value="{{ old("index_url", $row->index_url) }}" />
        </div> --}}
        ...
        <div class="col-md-4 mb-3">
            ...
            <div class="input-group">
                ...
                <input id="index_url" dir="ltr" required class="form-control text-left" type="text" name="index_url" value="{{ old('index_url', $row->index_url) }}" />
            </div>
        </div>
        
        <div class="col-md-5 mb-3">
            <label>{{ __('global.rewaq.book.promo_url') }} <strong class="text-danger">*</strong></label>
            <input required dir="ltr" type="url" class="form-control" name="promo_url" required value="{{ old("promo_url", $row->promo_url) }}" />
        </div>
                    ...
                    <div class="tab-content">
                        ...
                        @foreach($row->translations as $val)
                            <div class="tab-pane {{ ($val->locale==appLangKey())? 'active' : '' }}" id="tab-{{$val->locale}}">
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <label>{{ __('global.rewaq.book.name') }} ({{ LangNative($val->locale) }}) <strong class="text-danger">*</strong></label>
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <label>{{ __('global.rewaq.book.tags') }} ({{ LangNative($val->locale) }})</label>
                                        <br />
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <label>{{ __('global.rewaq.book.description') }} ({{ LangNative($val->locale) }}) <strong class="text-danger">*</strong></label>
                                    ...
                                    <div class="col-md-12 mb-3">
                                        <label>{{ __('global.rewaq.book.content') }} ({{ LangNative($val->locale) }}) <strong class="text-danger">*</strong></label>
