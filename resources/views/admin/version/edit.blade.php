...
@section('content')
<form action="{{ url('/admin/versions/'.$row->id) }}" method="post">@csrf {{ method_field('PUT') }}
    <div class="row">
        ...
        <div class="col-md-4 mb-3">
            ...
            <select name="writer_id" class="form-control">
                @foreach ($writers as $writer)
                    <option @if($row->writer_id==$writer->id) selected @endif value="{{$writer->id}}">{{ $writer->translation->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="col-md-4 mb-3">
            ...
            <div class="input-group">
                <span class="input-group-btn">
                    <a id="img2" data-input="thumimg5" data-preview="holderimg5" class="btn btn-danger text-white">
                    <i class="fa-regular fa-file-pdf"></i> {{ __('global.choose') }}
                </a>
                </span>
                <input id="thumimg5" required dir="ltr" class="form-control" type="text" name="pdf" value="{{ old('pdf',$row->pdf) }}" />
            </div>
                            ...
                            <div class="tab-pane {{ ($val->locale==appLangKey())? 'active' : '' }}" id="tab-{{$val->locale}}">
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <label>{{ __('global.version.news_name') }} ({{ LangNative($val->locale) }}) <strong class="text-danger">*</strong></label>
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <label>{{ __('global.version.news_tags') }} ({{ LangNative($val->locale) }})</label>
                                        <br />
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <label>{{ __('global.version.news_description') }} ({{ LangNative($val->locale) }}) <strong class="text-danger">*</strong></label>
                                    ...
                                    <div class="col-md-12 mb-3">
                                        <label>{{ __('global.version.news_content') }} ({{ LangNative($val->locale) }}) <strong class="text-danger">*</strong></label>
