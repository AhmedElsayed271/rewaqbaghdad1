...
@section('title', __('global.version.news'))

@section('breadcrumb')
<li class="breadcrumb-item">@yield('title')</li>
<li class="breadcrumb-item"><a href="{{ url('/admin/versions/create') }}">{{__('global.version.news_add')}}</a></li>
@endsection
...
<div class="mb-2">
    <a class="btn btn-secondary" href="{{ url('/admin/versions/create') }}">{{__('global.version.news_add')}}</a>
</div>
...
<div class="table-responsive">
    <table class="table table-striped data-table text-md-nowrap" width="100%">
        <thead>
            <tr>
                <th>{{__('global.id')}}</th>
                <th>{{__('global.version.news_name')}}</th>
                <th>{{__('global.version.categor_name')}}</th>
                <th>{{__('global.version.writer_name')}}</th>
                <th>{{__('global.slug')}}</th>
                <th>{{__('global.version.created_at')}}</th>
    ...
    $(document).ready(function(){
        var table = $('.table').DataTable({
            ...
            columns: [
                ...
                {data: 'writer.name', name: 'writer.name'},
                {data: 'slug', name: 'slug'},
            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, row, meta) {
                        return "<a target='_blank' href='{{url('/')}}/version/"+data+"'><i class='fa-solid fa-up-right-from-square'></i></a>";
                    }
                },
                {
                    targets: 5,
