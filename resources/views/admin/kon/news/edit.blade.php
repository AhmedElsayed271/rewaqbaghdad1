...
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ url('/admin/kon-news') }}">{{ __('global.media.news.title') }}</a></li>
    <li class="breadcrumb-item">@yield('title')</li>
@endsection

@section('content')
<form action="{{ url('/admin/kon-news/'.$row->id) }}" method="post">@csrf {{ method_field('PUT') }}
    <div class="row">
        ...
        <div class="col-md-6 mb-3">
            ...
            <div class="input-group">
                ...
                <input id="thumimg2" required dir="ltr" class="form-control" type="text" name="thum_img" value="{{ old('thum_img', $row->thum_img) }}" />
            </div>
            <div id="holderimg2" style="margin-top:15px;max-height:100px;">
                <img src="{{ old('thum_img', $row->thum_img) }}" style="height: 5rem;">
            </div>
        </div>
        ...
        <div class="col-md-6 mb-3">
            ...
            <div class="input-group">
                ...
                <input id="thumimg3" required dir="ltr" class="form-control" type="text" name="slider_img" value="{{ old('slider_img', $row->slider_img) }}" />
            </div>
            <div id="holderimg3" style="margin-top:15px;max-height:100px;">
                <img src="{{ old('slider_img', $row->slider_img) }}" style="height: 5rem;">
            </div>
        </div>
                    ...
                    <div class="tab-content">
                        ...
                        @foreach($row->translations as $val)
                            <div class="tab-pane {{ ($val->locale==appLangKey())? 'active' : '' }}" id="tab-{{$val->locale}}">
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <label>{{ __('global.media.news.name') }} ({{ LangNative($val->locale) }}) <strong class="text-danger">*</strong></label>
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <label>{{ __('global.media.news.tags') }} ({{ LangNative($val->locale) }})</label>
                                        <br />
                                        <input name="tags[{{$val->locale}}]" value="{{ old("tags")[$val->locale] ?? $val->tags }}" class="tag_{{$val->locale}}" placeholder="{{ __('global.media.news.tags') }}" />
                                    </div>

                                    <div class="col-md-6 mb-3">
                                        <label>{{ __('global.media.news.description') }} ({{ LangNative($val->locale) }}) <strong class="text-danger">*</strong></label>
                                    ...
                                    <div class="col-md-12 mb-3">
                                        <label>{{ __('global.media.news.content') }} ({{ LangNative($val->locale) }}) <strong class="text-danger">*</strong></label>
