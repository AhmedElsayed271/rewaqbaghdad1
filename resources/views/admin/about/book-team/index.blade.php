...
@section('title', __('global.book_team.title'))

@section('breadcrumb')
<li class="breadcrumb-item">@yield('title')</li>
<li class="breadcrumb-item"><a href="{{ url('/admin/book-team/create') }}">{{__('global.book_team.add_new')}}</a></li>
@endsection
...
<form action="{{url('/admin/book-team/update/description')}}" class="row mb-2" method="post">@csrf
    ...
    <div class="col-md-12 text-center">
        ...
        <a class="btn btn-success" href="{{ url('/admin/book-team/create') }}">{{__('global.book_team.add_new')}}</a>
    </div>
</form>
...
<div class="table-responsive">
    <table class="table table-striped data-table text-md-nowrap" width="100%">
        <thead>
            <tr>
                <th>{{__('global.id')}}</th>
                <th>{{__('global.book_team.name')}}</th>
                <th>{{__('global.book_team.job_title')}}</th>
                <th>{{__('global.book_team.email')}}</th>
        ...
        var table = $('.table').DataTable({
            ...
            columnDefs: [
                ...
                },
                {
                    targets: 5,
                    render: function (data, type, row, meta) {
                        var edit = '<a href="{{ url('/admin/book-team') }}/'+row.id+'/edit" class="btn mb-1 btn-sm btn-info"><i class="fa-fw fas fa-pen-alt"></i></a>';
                        var del = '<form style="display:inline-block" action="{{url('/admin/book-team')}}/'+row.id+'" method="post">@csrf {{ method_field('DELETE') }} <button onclick="if(confirm(`{{__("global.alert_delete")}}`)){return true;}else{return false;}" class="btn btn-sm mb-1 btn-danger"><i class="far fa-fw fa-trash-alt"></i></button></form>';
                        var btns = '';
                        @if(auth('admin')->user()->hasPermission('update-bookTeam') || auth('admin')->user()->is_superadmin)
                            btns += edit + ' ' ;
                         @endif
                        @if(auth('admin')->user()->hasPermission('delete-bookTeam') || auth('admin')->user()->is_superadmin)
                            btns += del;
                        @endif
                        return btns;
                    }
